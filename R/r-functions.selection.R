...
parse.aggregated.mut <- function(input.file.name, output.folder='', output.prefix='') {
	...
	for(ml in 1:length(mut.list)) {
		...
		if(!is.na(subject.name) & nrow(mut) > 0) {
			...
			m <- aggregate(chr ~ ensembl.gene + symbol + cds.pos + cds.ref + cds.alt + mut_type + type + prot.pos + prot.ref + prot.alt, data=mut, 'length');  ## @@@@@@@@ changed gene.id to ensembl.gene
	...
	close(log.file);
	symbol.2.cds.id = unique(symbol.2.cds.id)
...
check.gene.consv.all <- function(id, type='symbol') {
	refseq.id <- id;
	if (type == 'symbol') {
		refseq.id <- unique(refseq_map[which(refseq_map$symbol == id), 'protein.id'])
	} else if (type == 'gene.id') {
		refseq.id <- unique(refseq_map[which(refseq_map$gene.id == id), 'protein.id'])
	} else if (type == 'ensembl.cds') {
		refseq.id <- unique(refseq_map[which(refseq_map$cds.id == id), 'protein.id'])
	} else if (type == 'ensembl.protein') {
		refseq.id <- unique(refseq_map[which(refseq_map$protein.id == id), 'protein.id'])
	} else if (type == 'ensembl.gene') {
		refseq.id <- unique(refseq_map[which(refseq_map$ensembl.gene == id), 'protein.id'])
	}
...
reformat <- function(output.folder, output.prefix) {
	...
	opts.scs <- merge(opts.scs, unique(anno.length[, c('ensembl_transcript_id', 'ensembl.gene.id')]), by.x='cds.id', by.y='ensembl_transcript_id', all.x=T, all.y=F);
